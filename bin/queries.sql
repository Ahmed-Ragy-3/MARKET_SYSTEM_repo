...
UPDATE products
SET 
   IMAGE_URL = SUBSTR(IMAGES, 1, INSTR(IMAGES, ',', 1) - 1),
   IMAGES = TRIM(LEADING ',' FROM TRIM(SUBSTR(IMAGES, INSTR(IMAGES, ',', 1))))
...
SELECT IMAGE_URL, IMAGES FROM PRODUCTS WHERE IMAGE_URL is NULL;
...
UPDATE PRODUCTS
SET CATEGORY = 'Others' WHERE CATEGORY IS NULL;
